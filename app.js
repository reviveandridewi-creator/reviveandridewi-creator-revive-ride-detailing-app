...
function renderJobs(jobs) {
    if (!jobsTableBody || !archiveTableBody) return;
    ...
    jobs.forEach((job) => {
        ...
        let beforePreviews = (job.beforePhotos || []).map(src => `<img src="${src}" class="photo-thumb" onclick="window.open('${src}')" title="Click to enlarge" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px; margin-right: 4px;">`).join('');
        let afterPreviews = (job.afterPhotos || []).map(src => `<img src="${src}" class="photo-thumb" onclick="window.open('${src}')" title="Click to enlarge" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px; margin-right: 4px;">`).join('');

        let photosColumn = `
            <div class="photo-section" style="display: flex; flex-direction: column; gap: 6px;">
                <div class="photo-input-group">
                    <label class="camera-btn" style="cursor: pointer; font-size: 0.8rem; background: #21262d; padding: 4px 8px; border-radius: 4px; border: 1px solid #30363d; display: inline-block; margin-bottom: 4px;">
                        📷 Before Photo
        ...
        let statusOptions = `
            <select class="status-dropdown" onchange="window.updateStatus('${job.id}', this.value)" style="background: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 4px; padding: 4px 6px; margin-bottom: 6px; width: 100%;">
        ...
        let actionButtons = `
            ${statusOptions}
            <button class="action-btn ${archiveButtonClass} toggle-btn" onclick="window.toggleArchive('${job.id}', ${job.isArchived || false})" style="width: 100%;">${archiveButtonText}</button>
            <button class="action-btn delete-btn" onclick="window.deleteJob('${job.id}')" style="width: 100%;">Delete Log</button>
        `;
...
function updateMetrics(jobs) {
    ...
    if (activeJobsEl) activeJobsEl.textContent = dashboardCount;
...
window.toggleView = function(view) {
    ...
    if (btnActive) btnActive.classList.remove('active-tab');
    if (btnArchive) btnArchive.classList.remove('active-tab');
    if (btnRef) btnRef.classList.remove('active-tab');

    if (view === 'archive') {
        if (archivePanel) archivePanel.style.display = 'block';
        if (btnArchive) btnArchive.classList.add('active-tab');
    } else if (view === 'ref') {
        if (refPanel) refPanel.style.display = 'block';
        if (btnRef) btnRef.classList.add('active-tab');
    } else {
        if (activePanel) activePanel.style.display = 'block';
        if (btnActive) btnActive.classList.add('active-tab');
    }
};
...
window.toggleRefSubTab = function(subTab) {
    ...
    if (sopsContent) sopsContent.style.display = 'none';
    if (sdsContent) sdsContent.style.display = 'none';
    if (dilutionContent) dilutionContent.style.display = 'none';

    if (subBtnSops) subBtnSops.classList.remove('active-sub-tab');
    if (subBtnSds) subBtnSds.classList.remove('active-sub-tab');
    if (subBtnDilution) subBtnDilution.classList.remove('active-sub-tab');

    if (subTab === 'sds') {
        if (sdsContent) sdsContent.style.display = 'block';
        if (subBtnSds) subBtnSds.classList.add('active-sub-tab');
    } else if (subTab === 'dilution') {
        if (dilutionContent) dilutionContent.style.display = 'block';
        if (subBtnDilution) subBtnDilution.classList.add('active-sub-tab');
    } else {
        if (sopsContent) sopsContent.style.display = 'block';
        if (subBtnSops) subBtnSops.classList.add('active-sub-tab');
    }
